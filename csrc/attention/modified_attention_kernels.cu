...
template<
  ...
  int BLOCK_SIZE,
  int NUM_THREADS>
__global__ void modified_single_query_cached_kv_attention_kernel(
  scalar_t* __restrict__ out,             // [num_seqs, num_heads, head_size]
  float* __restrict__ attn,            // [num_seqs, num_heads, max_num_blocks_per_seq]
  ...
  const float scale,
  const int* __restrict__ block_tables,   // [num_seqs, layer_num, max_num_blocks_per_seq]
  ...
  //TODO (andy): make this a template parameter later
...
#pragma unroll
  ...
  // use layer_id to index into the block_tables
  const int* block_table = block_tables + seq_idx * num_layers * max_num_blocks_per_seq + layer_id * max_num_blocks_per_seq;
...
#pragma unroll
  ...
  const int attn_prefix = seq_idx * num_heads * max_num_blocks_per_seq + head_idx * max_num_blocks_per_seq;
  for (int i = thread_idx; i < context_len; i += NUM_THREADS) {
    logits[i] *= inv_sum;
    // Store the attention weights.
    attn[attn_prefix + i] = logits[i];
  }
  __syncthreads();
...
#define LAUNCH_ATTENTION_KERNEL(T, HEAD_SIZE, BLOCK_SIZE, NUM_THREADS)                        \
  vllm::modified_single_query_cached_kv_attention_kernel<T, HEAD_SIZE, BLOCK_SIZE, NUM_THREADS>        \
...
template<
  ...
  int BLOCK_SIZE,
  int NUM_THREADS = 128>
void modified_single_query_cached_kv_attention_launcher(
  torch::Tensor& out,
  torch::Tensor& attn,
  torch::Tensor& query,
  ...
  float* attn_ptr = reinterpret_cast<float*>(attn.data_ptr());
...
#define CALL_KERNEL_LAUNCHER(T, BLOCK_SIZE)                         \
  modified_single_query_cached_kv_attention_launcher<T, BLOCK_SIZE>(         \
    out,                                                            \
    attn,                                                           \
    query,                                                          \
    key_cache,                                                      \
...
void modified_single_query_cached_kv_attention(
  ...
  torch::Tensor& attn,            // [num_seqs, num_heads, max_context_len]
  torch::Tensor& query,           // [num_seqs, num_heads, head_size]
  torch::Tensor& key_cache,       // [num_blocks, num_heads, head_size/x, block_size, x]
  torch::Tensor& value_cache,     // [num_blocks, num_heads, head_size, block_size]
  float scale,
  torch::Tensor& block_tables,    // [num_seqs, num_layers, max_num_blocks_per_seq]
